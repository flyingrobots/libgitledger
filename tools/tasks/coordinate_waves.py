...
def parse_max_wave_gh(gh: GHCLI) -> int:
    # Probe waves from 1..50 and pick the highest that has at least one issue
    max_w = 0
    for w in range(1, 51):
        try:
...
def run_watcher_gh(wave: int, wave_issue: int | None = None) -> int:
    logs = Path('.slaps/logs')
    logs.mkdir(parents=True, exist_ok=True)
    out_f = (logs / 'watcher.out').open('a', encoding='utf-8')
    err_f = (logs / 'watcher.err').open('a', encoding='utf-8')
...
def main() -> int:
    ...
    def post_update(title: str, md: str) -> None:
        if project is None:
            return
        try:
            gh.project_item_create_draft(project, title, md)
            jsonl.emit('project_update', title=title)
        except Exception:
            pass
    ...
    for wave in range(wave_start, max_wave + 1):
        ...
        md = (
            f"## SLAPS Update\n\n"
            f"The **SWARM** is about to descend upon wave {wave} of {max_wave} 🦾 beep-boop.\n\n"
            f"```text\nProgress\n{bar(pct)}\nOn wave {wave} of {max_wave}\n```\n\n"
            f"(NOTE: This message was automatically generated by a SLAPS swarm.)\n"
        )
        post_update(f"Wave {wave} start", md)
        wave_issue_num = None
        if args.mode == 'gh' and project is not None:
            # Create Wave Status Issue and pass env down to watcher
        ...
        md2 = (
            f"## SLAPS Update\n\n"
            f"Wave {wave} completed.\n\n"
            f"```text\nProgress\n{bar(pct2)}\nOn wave {wave} of {max_wave}\n```\n\n"
            f"(NOTE: This message was automatically generated by a SLAPS swarm.)\n"
        )
        post_update(f"Wave {wave} complete", md2)
    ...
    if project is not None:
        try:
            md3 = (
                f"## SLAPS Update\n\n"
                f"All waves complete.\n\n"
                f"```text\nProgress\n{bar(100)}\nOn wave {max_wave} of {max_wave}\n```\n\n"
                f"(NOTE: This message was automatically generated by a SLAPS swarm.)\n"
            )
            gh.project_item_create_draft(project, f"All waves complete", md3)
        except Exception:
            pass
    return 0
